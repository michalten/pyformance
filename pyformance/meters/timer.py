...
class Timer(BaseMetric):
    ...
    def __init__(self, key, threshold=None, size=DEFAULT_SIZE, alpha=DEFAULT_ALPHA,
                 clock=time, sink=None, sample=None, tags=None):
        super(Timer, self).__init__(key, tags)
        self.meter = Meter(key=key, tags=tags, clock=clock)
        self.hist = Histogram(key=key, tags=tags, size=size, alpha=alpha, clock=clock, sample=sample)
        self.sink = sink
